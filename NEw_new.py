...
# MODIFIED: Now includes ALL DUIDs (both matched and unmatched with NEM data)
...
REGION_FILTER = ["NSW1"]
# REGION_FILTER = []  # Empty list means include ALL DUIDs regardless of region
    ...
    try:
        ...
        for _, row in nem_df_clean.iterrows():
            ...
            duid_lookup[duid] = {
                ...
                'Fueltech': row.get('Fuel Bucket Summary', 'N/A')
            }
...
def should_include_duid(duid, metadata, duid_lookup, region_filter):
    """
    ✅ MODIFIED: Check if a DUID should be included based on the region filter
    Now includes DUIDs even if they don't have region information
    """
    ...
    # ✅ NEW: If region is still N/A or Unknown, include it anyway
    # This ensures DUIDs without region info are not filtered out
    if region in ['N/A', 'Unknown', None, '']:
        print(f"📝 Including DUID {duid} despite unknown region")
        return True
                        ...
                        for result in facility_block.get("results", []):
                            ...
                            if duid in duid_lookup:
                                ...
                                enhanced_metadata = {
                                    ...
                                    "NEM_Matched": "Yes"  # ✅ NEW: Track if DUID was matched
                                }
                                ...
                                enhanced_metadata.update({
                                    ...
                                    "NEM_Matched": "No"  # ✅ NEW: Track if DUID was not matched
                                })
                                all_metadata[key] = enhanced_metadata

                            # ✅ MODIFIED: Apply region filter but with improved logic
...
def create_output_files(all_records, all_periods_metadata, reference_duids, decommissioned_duids, latest_period):
    ...
    # ✅ NEW: Show enhanced breakdown including matched vs unmatched DUIDs
    if reference_columns:
        ...
        for col in reference_columns:
            if col in latest_metadata:
                ...
                if latest_metadata[col].get('NEM_Matched', 'No') == 'Yes':
                    matched_count += 1
                else:
                    unmatched_count += 1
        ...
        print(f"✅ DUIDs matched with NEM data: {matched_count}")
        print(f"🆕 DUIDs from API only (unmatched): {unmatched_count}")
    ...
    if has_enhanced_data:
        meta_fields = ["DUID", "Name", "Facility", "Region", "Fueltech", 
                       ...
                       "Storage_Capacity_MWh", "Expected_Closure_Year", "NEM_Matched"]
    else:
        meta_fields = ["DUID", "Name", "Facility", "Region", "Fueltech", "NEM_Matched"]
...
def main():
    ...
    if REGION_FILTER:
        ...
        print("📝 Note: DUIDs without region info will still be included")
    else:
    ...
    if len(periods) == 1:
        ...
        # ✅ MODIFIED FILTERING LOGIC - Include ALL DUIDs (both matched and unmatched)
        print("🔄 Processing all DUIDs (both matched and unmatched)...")
        valid_columns = []
        for col in monthly_df.columns:
            if col in metadata:
                duid = metadata[col].get('DUID', 'N/A')
                # ✅ NEW: Include ALL DUIDs that have valid DUID values
                if duid != 'N/A' and duid != '' and duid is not None:
                    valid_columns.append(col)

        monthly_df = monthly_df[valid_columns]
        print(f"✅ Final matrix: {monthly_df.shape[0]} months × {monthly_df.shape[1]} DUIDs")
        ...
        if valid_columns:
            ...
            for col in valid_columns:
                if col in metadata:
                    ...
                    # Count matched vs unmatched
                    if metadata[col].get('NEM_Matched', 'No') == 'Yes':
                        matched_count += 1
                    else:
                        unmatched_count += 1
        ...
        # 🏷️ Add enhanced metadata as header rows (now including NEM_Matched field)
        ...
        print(f"   • All DUIDs included: ✅ Both matched and unmatched DUIDs")
        ...
        # Count matched vs unmatched in latest period
        latest_metadata = all_periods_metadata[latest_period]
        latest_matched = sum(1 for meta in latest_metadata.values() if meta.get('NEM_Matched') == 'Yes')
        latest_unmatched = sum(1 for meta in latest_metadata.values() if meta.get('NEM_Matched') == 'No')
        ...
        print(f"     - Matched with NEM data: {latest_matched}")
        print(f"     - API only (unmatched): {latest_unmatched}")
